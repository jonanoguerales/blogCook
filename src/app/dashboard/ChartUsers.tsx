...
const ChartUsers: React.FC = () => {
  const [dates, setDates] = useState<UserData[]>([]);
  ...
  const datesTotales = dates.map((date) =>
    new Date(date.createdAt).toDateString()
  );
  const datesTotales2 = dates.map((date) =>
    new Date(date.createdAt).getMonth()
  );
  const datesTotales3 = dates.map((date) =>
  ...
  const registradosHoy = datesTotales.filter(
    (datesTotales) => datesTotales === hoy
  );
  const registradosMes = datesTotales2.filter(
    (datesTotales2) => datesTotales2 === mesNumero
  );
  const registradosAño = datesTotales3.filter(
    (datesTotales3) => datesTotales3 === anoNumero
  );
